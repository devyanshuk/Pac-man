...
class Cheats:
    ...
    def pauseTheGhosts(self): self.game.pauseGhosts = True

    def moveGhosts(self): self.game.pauseGhosts = False

    def increaseLife(self): self.game.pac.lives = 3

    def pausePacman(self): self.game.pausePlayer = True

    def resumePacman(self): self.game.pausePlayer = False

    def makePacmanimmortal(self):self.game.immortal = True

    def makeGameSlow(self): self.game.fastMode = False

    def makePacmanMortal(self): self.game.immortal = False

    def makeGameFast(self): self.game.fastMode = True
    ...
    def normalGhostMovement(self): self.game.allTargetPacman = self.game.leavePacmanAlone = False
    ...
    def changeBackGround(self):
        if self.game.env.currentBg == self.game.env.background: self.game.env.currentBg = self.game.env.background2
        else: self.game.env.currentBg = self.game.env.background
