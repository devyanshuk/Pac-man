...
class Game:
    ...
    def __init__(self):
        ...
        self.pac = Player(self,vec(self.env.pPos))
        self.level = 3 #levels 1 to 3
        self.ghosts = [(Ghost(self,vec(self.env.gPos[i]),i)) for i in range(4)]
        self.text= ''
        ...
        self.currentScore = 0
        self.cheatDisplayTime = self.energizerTickCount = self.currChoice = self.winTickCount = self.gameTick = 0
    ...
    def update(self):
        ...
        self.updateCoins()
        if self.won: self.env.displayTwoMazes(self.winTickCount%20)
        else: self.env.displayLayout()
    ...
    def displayGhost(self,image):
        for i in range(4):
            if self.eyeUsed[i]: self.env.drawSprite(self.env.eye,self.ghosts[i].pixPos)
            else: self.env.drawSprite(image,self.ghosts[i].pixPos)

    def displayTwoGhostImages(self):
        a = self.energizerTickCount % 10
        if a<5: self.displayGhost(self.env.scared)
        else: self.displayGhost(self.env.scaredLight)

    def drawGhostIfEnergizerUsed(self):
        if not self.energizerUsed:
            for i in range(4):self.env.drawSprite(self.ghosts[i].colour,self.ghosts[i].pixPos)
        else:
            if self.energizerTickCount <150: self.displayGhost(self.env.scared)
            else: self.displayTwoGhostImages()

    def checkCollision(self):
        if not self.pauseGhosts:
            for ghost in self.ghosts: ghost.moveGhosts()
        for i in range(4):
            if self.ghosts[i].gridPos == self.pac.gridPos and not self.eyeUsed[i]:
                if not self.immortal and not self.energizerUsed: self.removeLife()
            ...
            if self.ghosts[i].inCentreOfGrid():
                if self.cheat.tooEasyUsed or self.eyeUsed[i]: self.ghosts[i].speed = 4
            ...
            if self.pauseGhosts and self.eyeUsed[i]: self.ghosts[i].moveGhosts()
        if self.collisionTick<=15: self.collisionTick+=1
    ...
    def waitForInput(self):
        while True:
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                if self.over: self.reset()
                self.play = True
                self.beginScreen = False
                break

    def checkCheat(self): self.cheat.checkForCheats(self.text)
    ...
    def resetPacmanAndGhosts(self):
        ...
        for i in range(4): self.resetGhost(self.ghosts[i],i)
    ...
    def removeLife(self):
        ...
        else: self.resetPacmanAndGhosts()
    ...
    def resetGhost(self,ghost,i):
        if self.level == 3: ghost.speed = ghost.setSpeed()
